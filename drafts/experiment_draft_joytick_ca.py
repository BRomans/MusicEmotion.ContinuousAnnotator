...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\miche\\EIT\\INTERNSHIP_MYBRAIN\\experimental_phase\\experiment_continuous_annotation\\experiment_draft_joytick_ca.py',
...
joystick_target = visual.ImageStim(
    win=win,
    name='joystick_target', units='norm', 
...
for thisMusic_emotion_trial_eo_ec in music_emotion_trial_eo_ec:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if joystick.status == STARTED:  # only update if started and not finished!
            ...
            [logging.data(
                "joystick_{}_button: {}, pos=({:1.4f},{:1.4f})".format(joystick.device_number, i, joystick.getX(),
                                                                       joystick.getY()) for i in
                joystick.pressedButtons)]
            x, y = clamp_stick(joystick.getX(), joystick.getY())
            joystick.x = x
            joystick.y = y
            new_target_position = [clamp_stick(joystick.get_left_thumbstick_axis()[0], -joystick.get_left_thumbstick_axis()[1])]
