...
expName = 'experiment_eo_ec_mouse'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\miche\\EIT\\INTERNSHIP_MYBRAIN\\experimental_phase\\experiment_continuous_annotation\\experiment_draft_mouse_ca.py',
...
mouse_annotation = event.Mouse(win=win)
x, y = [None, None]
mouse_annotation.mouseClock = core.Clock()
...
for thisMusic_emotion_trial_eo_ec in music_emotion_trial_eo_ec:
    ...
    # setup some python lists for storing info about the mouse_annotation
    mouse_annotation.x = []
    mouse_annotation.y = []
    mouse_annotation.leftButton = []
    mouse_annotation.midButton = []
    mouse_annotation.rightButton = []
    mouse_annotation.time = []
    gotValidClick = False  # until a click is received
    mouse_annotation.mouseClock.reset()
    ...
    # keep track of which components have finished
    trialComponents = [target, song_one, valence_arousal_space, mouse_annotation, open_eyes, song_one_rating, white_noise_rating, instruction_ec, song_two, valence_arousal_space_two, eyes_open, song_two_rating, white_noise_rating_two, instruction_eo]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if target.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > target.tStartRefresh + 45.0-frameTolerance:
                # keep track of stop time/frame for later
        ...
        # *mouse_annotation* updates
        if mouse_annotation.status == NOT_STARTED and t >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            mouse_annotation.frameNStart = frameN  # exact frame index
            mouse_annotation.tStart = t  # local t and not account for scr refresh
            mouse_annotation.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(mouse_annotation, 'tStartRefresh')  # time at next scr refresh
            mouse_annotation.status = STARTED
            prevButtonState = mouse_annotation.getPressed()  # if button is down already this ISN'T a new click
        if mouse_annotation.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > mouse_annotation.tStartRefresh + 45.0-frameTolerance:
                # keep track of stop time/frame for later
                mouse_annotation.tStop = t  # not accounting for scr refresh
                mouse_annotation.frameNStop = frameN  # exact frame index
                win.timeOnFlip(mouse_annotation, 'tStopRefresh')  # time at next scr refresh
                mouse_annotation.status = FINISHED
        if mouse_annotation.status == STARTED:  # only update if started and not finished!
            x, y = mouse_annotation.getPos()
            target.pos = [x, y]
            mouse_annotation.x.append(x)
            mouse_annotation.y.append(y)
            buttons = mouse_annotation.getPressed()
            mouse_annotation.leftButton.append(buttons[0])
            mouse_annotation.midButton.append(buttons[1])
            mouse_annotation.rightButton.append(buttons[2])
            mouse_annotation.time.append(mouse_annotation.mouseClock.getTime())
    ...
    for thisComponent in trialComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    music_emotion_trial_eo_ec.addData('target.started', target.tStartRefresh)
    music_emotion_trial_eo_ec.addData('target.stopped', target.tStopRefresh)
    ...
    music_emotion_trial_eo_ec.addData('mouse_annotation.x', mouse_annotation.x)
    music_emotion_trial_eo_ec.addData('mouse_annotation.y', mouse_annotation.y)
    music_emotion_trial_eo_ec.addData('mouse_annotation.leftButton', mouse_annotation.leftButton)
    music_emotion_trial_eo_ec.addData('mouse_annotation.midButton', mouse_annotation.midButton)
    music_emotion_trial_eo_ec.addData('mouse_annotation.rightButton', mouse_annotation.rightButton)
    music_emotion_trial_eo_ec.addData('mouse_annotation.time', mouse_annotation.time)
    music_emotion_trial_eo_ec.addData('mouse_annotation.started', mouse_annotation.tStart)
    music_emotion_trial_eo_ec.addData('mouse_annotation.stopped', mouse_annotation.tStop)
